...
    }
    /* GŁÓWNA KARTA DNIA Z OBRAMOWANIEM NA CAŁOŚĆ */
    .karta-dnia {
        border: 2px solid; 
        border-radius: 12px; 
        padding: 12px; 
...
        
        # --- MODYFIKACJA 1: STYLOWANIE DANYCH ---
        # Kolorowanie wierszy z kraj='Słowacja' na jasnozielono
        def style_sk(row):
...
                
                # --- MODYFIKACJA 2: JEDNO OBRAMOWANIE DLA CAŁEJ KARTY ---
                # Tworzymy główny kontener HTML dla całej karty z obramowaniem
                html = f"""<div class='karta-dnia' style='border-color:{c_h}'>
                    <div style='display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;'>
                        <div class='{badge_class}'><b style='color:{c_h}; font-size:14px;'>{dk} ({d['dz']})</b></div>
                        <div style='text-align: center;'><b style='color:#d32f2f; font-size:14px;'>PRZ: {d['prz']}</b></div>
                        <button class='btn-del' onclick='document.getElementById("btn_{dk}").click()'>❌</button>
                    </div>"""
                
                # Ukryty natywny przycisk Streamlit do obsługi usuwania
                if st.button("❌", key=f"btn_{dk}", type="secondary", help="Usuń dzień", on_click=lambda dk=dk: set_session_del_day(dk)): pass
                # Logika usuwania (dodana po kliknięciu ukrytego przycisku)
                if st.session_state.get(f"del_day_{dk}"):
...
                    del st.session_state[f"del_day_{dk}"]
                    st.rerun()
...
# Pomocnicza funkcja do obsługi stanów przycisku usuwania
def set_session_del_day(dk):
    st.session_state[f"del_day_{dk}"] = True
